...
PRED_OUT = OUTDIR / "structural_thermostability_ml_predictions.csv"
SUMMARY_OUT = OUTDIR / "structural_thermostability_ml_summary.csv"
FEATURE_OUT = OUTDIR / "structural_thermostability_ml_feature_importance.csv"
PERM_OUT = OUTDIR / "structural_thermostability_ml_permutation_importance.csv"
AUDIT_OUT = REPORTDIR / "structural_thermostability_ml_audit.txt"
...
NUMERIC_FEATURES = [
    ...
    # Structural/model quality features
    "chain_length",
...
def main():
    ...
    numeric_features = [c for c in NUMERIC_FEATURES if c in model_df.columns and c not in EXCLUDE_FROM_FEATURES]
    categorical_features = [c for c in CATEGORICAL_FEATURES if c in model_df.columns and c not in EXCLUDE_FROM_FEATURES]
    ...
    for model_name, regressor in models.items():
        ...
        pred_table["prediction_error"] = pred_table[TARGET] - pred_table["predicted_foldx_energy_per_residue"]
    ...
    # Additional grouped prediction summaries
    group_summary = (
    ...
    group_summary.to_csv(OUTDIR / "structural_thermostability_ml_group_summary.csv", index=False)

    audit = []
    audit.append("Structural Thermostability ML Audit")
    audit.append("=" * 80)
    ...
    print(f"Saved: {OUTDIR / 'structural_thermostability_ml_group_summary.csv'}")
